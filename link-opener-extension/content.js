...
const safeStorage = {
    ...
    remove: async (keys) => {
        ...
        try {
            ...
            console.warn("Link Batch Opener: Storage remove failed.", e);
        }
    }
};
...
// Matches SELECTED_VIDEOS_PREFIX in tiktok_ui.js - the per-profile TikTok
// video-selection storage. Kept here too so this page can offer a one-off
// bulk clear without needing to visit every profile individually.
const TIKTOK_SELECTED_VIDEOS_PREFIX = "tmk_selected_videos:";
// Matches FP_LOG_KEY in stagger_nav.js - the automatic-load false-positive
// diagnostic log.
const TIKTOK_FP_LOG_KEY = "tmk_fp_log";

async function clearAllTikTokVideoSelections() {
    const all = await safeStorage.get(null);
    const keys = Object.keys(all).filter(k => k.startsWith(TIKTOK_SELECTED_VIDEOS_PREFIX));
...
async function saveSelections() {
    const selected = getSelectedLinkKeys();
...
async function loadSelections() {
    ...
    checkboxes.forEach(cb => {
        const key = cb.dataset.origHref || cb.dataset.href;
        if (key && selectedUrls.includes(key)) {
            cb.checked = true;
        }
    });
    updateUI();
...
// Used for persistence (saving/restoring selection state) instead of
// getSelectedLinks(), since dataset.href may be rewritten to an opener
// target (e.g. " #" links -> ssstiktok.dev) that won't match the link's
// real href on the page and would fail to be restored/pruned correctly.
function getSelectedLinkKeys() {
    return Array.from(document.querySelectorAll(".link-checkbox:not(:disabled):checked"))
        .map(cb => cb.dataset.origHref || cb.dataset.href)
        .filter(Boolean);
}
...
async function addCheckboxes() {
    ...
    for (const link of links) {
        ...
        // origHref is the untransformed link URL as it appears in the page.
        // It's what we persist/match selections against, since it's stable
        // across reloads. dataset.href can get rewritten below (e.g. " #"
        // links point it at ssstiktok.dev) and is what's used for opening.
        cb.dataset.origHref = link.href;
        ...
        if (decodedHref.includes(" #")) {
            // Hide the " #" marker from display either way (handle encoded space too)
            link.textContent = link.textContent.replace(/(\s+|%20)#$/, "");

            const isTikTokLink = /tiktok\.com/i.test(decodedHref);

            if (isTikTokLink) {
                // TikTok profile links: redirect through ssstiktok.dev so all
                // of that account's videos can be found/downloaded at once.
                link.onclick = (e) => {
                    e.preventDefault();
                    e.stopPropagation();
            ...
            // Non-TikTok " #" links (other domains) get no special
            // click/open override - link.onclick stays default and
            // cb.dataset.href stays the real URL, so they use the exact
            // same open/select/category/staggered-load system as any other
            // ("main") link. Only the " #" marker in the displayed text is
            // hidden.
        }
    }
}
